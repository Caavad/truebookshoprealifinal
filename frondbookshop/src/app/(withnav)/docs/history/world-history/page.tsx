...
export default async function WorldHistoryPage() {
  ...
  const worldHistoryBooks: Book[] = allBooks; // Filtering by subcategory not yet supported by API
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <section className="mb-8">
        <nav className="text-sm text-gray-400 mb-4">
          ...
          <Link href="/docs/history" className="hover:text-white">History</Link>
          <span className="mx-2">/</span>
          <span className="text-white">World History</span>
        </nav>
        <h1 className="text-4xl font-bold mb-2">World History</h1>
        <p className="text-gray-400 text-lg">
          Explore civilizations and key historical events
        </p>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold">World History Books</h2>
          <div className="text-sm text-gray-400">
            {worldHistoryBooks.length} books available
          </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {worldHistoryBooks.map((book: Book) => (
