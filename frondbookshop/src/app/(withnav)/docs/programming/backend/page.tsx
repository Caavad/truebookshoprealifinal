...
export default async function BackendPage() {
  ...
  const backendBooks: Book[] = allBooks; // Filtering by subcategory not yet supported by API
    ...
    <main className="container mx-auto px-4 py-8">
      ...
      <section className="mb-8">
        ...
        <p className="text-gray-400 text-lg">
          Learn server-side development, APIs, and database management
        </p>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold">Backend Books</h2>
          <div className="text-sm text-gray-400">
            {backendBooks.length} books available
          </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {backendBooks.map((book: Book) => (
