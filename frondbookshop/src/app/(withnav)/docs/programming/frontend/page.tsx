...
export default async function FrontendPage() {
  ...
  const frontendBooks: Book[] = allBooks; // Filtering by subcategory not yet supported by API
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <section className="mb-8">
        ...
        <h1 className="text-4xl font-bold mb-2">Frontend Development</h1>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold">Frontend Books</h2>
          <div className="text-sm text-gray-400">
            {frontendBooks.length} books available
          </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {frontendBooks.map((book: Book) => (
